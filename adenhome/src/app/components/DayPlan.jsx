...
export default function DayPlan({
  ...
  return (
    <div
      className={`-mx-4 w-[calc(100%+2rem)] rounded-none border border-[#1B2C36] bg-[#0B1B24] p-4 lg:mx-0 lg:w-auto lg:rounded-2xl lg:p-6 ${
        fillHeight ? "flex h-full min-h-0 flex-col" : ""
      }`}
    >
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-semibold">Day Plan</h2>
        <span className="text-xs uppercase tracking-[0.2em] text-[#7FA5AE]">
      ...
      >
        <div className="flex gap-3" style={{ minHeight: timelineHeight }}>
          <div className="relative w-8 shrink-0 lg:w-12" style={{ height: timelineHeight }}>
            {timelineHours.map((hour, index) => (
              <div
                ...
                className="absolute left-0 text-[9px] uppercase tracking-[0.12em] text-[#7FA5AE] lg:text-[10px] lg:tracking-[0.2em]"
            ...
            {timelineHours.slice(0, 24).map((hour, index) => {
              ...
              return (
                <button
                  ...
                  className={`absolute left-0 right-0 rounded-md border border-[#0B1A22] ${
          ...
          <div
            className="relative flex-1 rounded-2xl border border-[#1B2C36] bg-[#08141C]"
          ...
          >
              ...
              {calendarEvents.map((event) => {
                ...
                return (
                  <div
                    ...
                    } ${isExpanded ? "border-transparent bg-transparent" : "border-[#1E4B5C] bg-[#0C2430]/90"} relative overflow-visible`}
                    style={{
                  ...
                  >
                    {isExpanded && (
                      <div
                        className={`absolute inset-x-0 top-0 border border-[#1E4B5C] bg-[#0C2430]/90 px-2 py-1 text-xs text-[#D6F1F4] pointer-events-none box-border ${expandedRadiusClass}`}
                        style={{ height: displayHeight }}
                      >
                        <div className="flex items-baseline justify-between gap-3 leading-tight">
                          <p className="text-sm font-semibold">{event.title}</p>
                          <p className="text-[11px] uppercase tracking-[0.2em] text-[#8ED0DA]">
                    ...
                    {!isExpanded && showText && (
                      <div className="px-2 py-1 text-[#D6F1F4]">
                        <div className="flex items-baseline justify-between gap-3 leading-tight">
                          <p className="text-sm font-semibold">{event.title}</p>
                          <p className="text-[11px] uppercase tracking-[0.2em] text-[#8ED0DA]">
                            {formatMinutes(event.startMinutes)} -{" "}
            ...
            {timelineHours.slice(0, 24).map((_, index) => (
              <div
                key={`grid-${index}`}
                className="absolute left-0 right-0 border-t border-[#0B1A22]"
            ...
            >
              {plannedBlocks.map((block) => {
                ...
                const baseClasses = isSelected
                  ...
                  : "border-[#27404A] bg-[#0B1A20]/80 text-[#E6F4F7]";
                            ...
                            <div className="flex items-center gap-1">
                              <button
                                ...
                                className={`rounded-full border px-1 py-0 text-[10px] ${
                                  moveAvailability?.get(block.taskId)?.canMoveUp
                                    ? "border-[#27404A] text-[#C8E5EA] hover:border-[#2CB8C8]"
                ...
                return (
                  <button
                    ...
                    className={`absolute left-0 w-3 rounded-r-md border pointer-events-auto ${
                      isSelected
                        ? "border-[#8ED0DA] bg-[#1E4B5C]"
                        : "border-[#1E4B5C] bg-[#0C2430]/90 hover:bg-[#233040]"
                    }`}
              ...
              {plannedBlocks.map((block) => {
                ...
                return (
                  <button
                    ...
                    onClick={() => onSelectTask(block.taskId)}
                    className={`absolute left-0 w-3 rounded-r-md border pointer-events-auto ${
                      isSelected ? "border-[#2CB8C8]" : "border-[#27404A]"
                    } ${modeColor}`}
                    style={{ top: tabTop, height: tabHeight }}
            ...
            <div className="absolute inset-x-4 bottom-4 rounded-xl border border-dashed border-[#27404A] p-3 text-xs text-[#7FA5AE]">
